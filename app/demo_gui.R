    ...
    tabPanel("Barcelona HF Score",
             sidebarLayout(
               sidebarPanel(
                 ...
                 numericInput("loop_diuretic_furosemide", "Loop Diuretic Furosemide Dose (mg)", value = 20, min = 0, max = 500),
                 numericInput("loop_diuretic_torasemide", "Loop Diuretic Torasemide Dose (mg)", value = 0, min = 0, max = 500),
...
server <- function(input, output) {
  ...
  observeEvent(input$calculate_barcelona, {
    ...
    barcelona_parameters <- list(
      ...
      `Loop Diuretic Furosemide Dose` = as.numeric(input$loop_diuretic_furosemide),
      `Loop Diuretic Torasemide Dose` = as.numeric(input$loop_diuretic_torasemide),
      Statin = input$statin,
      `ACEi/ARB` = input$acei_arb,
      Betablockers = input$betablocker,
      `HF Duration in months` = as.numeric(input$hf_duration),
      `Diabetes Mellitus` = input$diabetes,
